...
uses dialogue text without establishing contextual appropriateness. These
results show how action mediation exposes an alignment--repetition trade-off
that fluent utterances or aggregate similarity measures can conceal.
\end{abstract}
...
whether rationales carry action-relevant information, and how the allocation
of action control shapes observable knowledge introduction, connection, and
development across the resulting long-horizon dialogues.
...
\begin{itemize}[leftmargin=*,itemsep=1pt,topsep=2pt]
    ...
    expose an alignment--repetition trade-off that aggregate similarity alone
    conceals and provide an explicit boundary for examining downstream
...
rationale--action consistency (G3), and observable knowledge-construction
patterns in generated dialogue (G4).
...
The knowledge-construction operationalization for the revised G4 is being
...
proposals and reduces these repetitions, but moves the resulting trajectories
away from the held-out action distribution. Because the configurations also differ in
...
repeats short cycles and motifs more often. These findings position human-grounded
